module ViewComponentReflex::Form::DateInputs
  def day_select(attribute, **options)
    ...
    select(_input_name_for(attribute), combine_options(default_options, options))
  end
  ...
  def date_input(attribute, **options)
    ...
    date_field_tag(_input_name_for(attribute), record.send(attribute), combine_options(default_options, options))
  end

  def month_input(attribute, **options)
    default_options = { class: ['form-control'], data: input_reflex_data_attributes }

    month_field_tag(_input_name_for(attribute), record.send(attribute), combine_options(default_options, options))
  end
end
